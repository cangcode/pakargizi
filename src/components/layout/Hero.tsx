...
const Hero = async () => {
  ...
  return (
    <div className='flex flex-col justify-center md:items-center md:text-center px-4 gap-2 mt-10'>
        <h1 className='font-bold text-primGreen text-4xl md:text-7xl '>Halo{!session && '!'} <span className='uppercase text-secGreen'>{session && session?.user.name + '!'}</span> welcome to <span className='text-primGreen font-bold'>Nutri Ta'</span></h1>
